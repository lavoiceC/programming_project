...
//bia detection
    var biasFound = rules.bias_terms.filter(function(term) {
        return jdLower.indexOf(term.toLowerCase()) > -1;
    });

    var biasOutput;
        if (biasFound.length > 0) {
            biasOutput = biasFound.map(function(term) {
                var type = rules.bias_types[term] || "General Bias";
                return term + " (" + type + ")";
            }).join(", ");
        } else {
            biasOutput = "bias not detected";
        }
    ...
    document.getElementById("bias").textContent = biasOutput;
